...
// API abstracts the API for interfacing NRI from the rest of cri-o. It is
// agnostic to any internal implementation details of pods and containers.
// A corresponding Domain interface, with knowledge about implementation
// details is registered by server.Server. Since we only need to deal with
// containers in the CRI 'domain', this intermediate abstraction and the
// split to two separate interfaces, one NRI 'downward' and the other
// 'upward' is technically artificial. We have these in place to keep
// the NRI adaptation code structurally close to that of other runtimes,
// in the hope of lowering the mental cost of co-maintaining them.
type API interface {

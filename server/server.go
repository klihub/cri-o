...
import (
	...
	nriIf "github.com/cri-o/cri-o/nri"
)
...
func New(
	...
	cfg := s.Config().NRI
	if cfg.Enabled {
		api, err := nriIf.New(&nriIf.Config{
			Disable:    false,
...
func (s *Server) handleExit(ctx context.Context, event fsnotify.Event) {
	...
	if nriCtr != nil && s.nriApi.isEnabled() {
		s.nriApi.stopContainer(ctx, nil, nriCtr)
	}

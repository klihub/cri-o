...
type nriRuntime struct {
	...
	nri    *nri.Adaptation // NRI runtime abstraction
...
func (n *nriRuntime) CreateContainer(ctx context.Context, spec *specs.Spec, octr *oci.Container, pod *sandbox.Sandbox) (*nri.ContainerAdjustment, error) {
	...
	request := &nri.CreateContainerRequest{
		Pod:       podSandboxToNRI(pod),
		Container: containerToNRI(octr, spec),
	}
...
func (n *nriRuntime) PostCreateContainer(ctx context.Context, octr *oci.Container, pod *sandbox.Sandbox) error {
	...
	request := &nri.PostCreateContainerRequest{
		Pod:       podSandboxToNRI(pod),
		Container: containerToNRI(octr, nil),
	}
...
func podSandboxToNRI(pod *sandbox.Sandbox) *nri.PodSandbox {
	crio := &crioPodSandbox{
		Sandbox: pod,
	}
	return &nri.PodSandbox{
		...
		Name:           crio.GetName(),
		Uid:            crio.GetUID(),
		Namespace:      crio.GetNamespace(),
		Annotations:    crio.GetAnnotations(),
		Labels:         crio.GetLabels(),
		CgroupParent:   crio.GetCgroupParent(),
		RuntimeHandler: crio.GetRuntimeHandler(),
	}
}

func containerToNRI(ctr *oci.Container, spec *specs.Spec) *nri.Container {
	crio := &crioContainer{
		ctr:  ctr,
		spec: spec,
	}
	return &nri.Container{
		Id:           crio.GetID(),
		PodSandboxId: crio.GetPodSandboxID(),
		Name:         crio.GetName(),
		State:        crio.GetState(),
		Labels:       crio.GetLabels(),
		Annotations:  crio.GetAnnotations(),
		Args:         crio.GetArgs(),
		Env:          crio.GetEnv(),
		Mounts:       crio.GetMounts(),
		Hooks:        crio.GetHooks(),
		Linux: &nri.LinuxContainer{
			Namespaces:  crio.GetLinuxNamespaces(),
			Devices:     crio.GetLinuxDevices(),
			Resources:   crio.GetLinuxResources(),
			OomScoreAdj: nri.Int(crio.GetOOMScoreAdj()),
			CgroupsPath: crio.GetCgroupsPath(),
		},
	}
}
...
func (c *crioContainer) GetState() nri.ContainerState {
	if c.ctr != nil {
		...
		case oci.ContainerStateCreated:
			return nri.ContainerState_CONTAINER_CREATED
		case oci.ContainerStatePaused:
			return nri.ContainerState_CONTAINER_PAUSED
		case oci.ContainerStateRunning:
			return nri.ContainerState_CONTAINER_RUNNING
		case oci.ContainerStateStopped:
			return nri.ContainerState_CONTAINER_STOPPED
		}
	}

	...
	wrapgen := nrigen.SpecGenerator(specgen,
		nrigen.WithLabelFilter(
			func(values map[string]string) (map[string]string, error) {
				...
				err := a.cri.FilterDisallowedAnnotations(annotations, values, handler)
				if err != nil {
					return nil, fmt.Errorf("disallowed labels in NRI adjustment: %w", err)
				}
				return values, nil
			},
		),
...
func (a *nriAPI) undoCreateContainer(ctx context.Context, specgen *generate.Generator, criPod *sandbox.Sandbox, criCtr *oci.Container) {
	...
	err := a.nri.StopContainer(ctx, pod, ctr)
	if err != nil {
		log.Errorf(ctx, "failed to creation undo (stop) failed: %v", err)
	}

	err = a.nri.RemoveContainer(ctx, pod, ctr)
	if err != nil {
		log.Errorf(ctx, "failed to creation undo (remove) failed: %v", err)
	}
}
...
func (a *nriAPI) ListContainers() []nri.Container {
	...
	ctrList, _ := a.cri.ContainerServer.ListContainers()
...
func (a *nriAPI) GetContainer(id string) (nri.Container, bool) {
	ctr, err := a.cri.GetContainerFromShortID(todoMissingContext(), id)
	if err != nil {
		return nil, false
	}
...
func (a *nriAPI) UpdateContainer(ctx context.Context, u *api.ContainerUpdate) error {
	ctr, err := a.cri.GetContainerFromShortID(todoMissingContext(), u.ContainerId)
	if err != nil {
	...
	err = a.cri.Runtime().UpdateContainer(ctx, ctr, resources)
	if err != nil {
		log.Errorf(ctx, "failed to update CRI container %q: %v", u.ContainerId, err)
		if u.IgnoreFailure {
			return nil
...
func (a *nriAPI) EvictContainer(ctx context.Context, e *api.ContainerEviction) error {
	ctr, err := a.cri.GetContainerFromShortID(todoMissingContext(), e.ContainerId)
	if err != nil {
		// We blindly assume container with given ID not found and ignore it.
...
func todoMissingContext() context.Context {
	// TODO(klihub): call sites of this should receive the context as an
	// argument and use that instead (typically for StartSpan()/End())
...
func (c *criPodSandbox) GetDomain() string {
	return nriDomain
}
...
func (p *criPodSandbox) GetPodLinuxOverhead() *api.LinuxResources {
	...
	//return api.FromCRILinuxResources(p.CRISandbox().GetConfig().GetLinux().GetOverhead())
	return nil
}
...
func (p *criPodSandbox) GetPodLinuxResources() *api.LinuxResources {
	...
	// TODO(klihub): AFAICT, CRI PodSandboxConfig is not stored, so we can only
	// get our hands on it during RunPodSandbox() request processing...
	//return api.FromCRILinuxResources(p.CRISandbox().GetConfig().GetLinux().GetResources())
	return nil
}

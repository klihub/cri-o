...
func mergeConfig(config *libconfig.Config, ctx *cli.Context) error {
	...
	if ctx.IsSet("nri-enable-default-validator") {
		config.NRI.DefaultValidator.DefaultValidatorConfig.Enable = ctx.Bool("nri-enable-default-validator")
	}
	...
	if ctx.IsSet("nri-validator-reject-oci-hook-adjustment") {
		config.NRI.DefaultValidator.DefaultValidatorConfig.RejectOCIHookAdjustment = boolptr(
			ctx.Bool("nri-validator-reject-oci-hook-adjustment"),
		)
	}

	if ctx.IsSet("nri-validator-reject-runtime-default-seccomp-adjustment") {
		config.NRI.DefaultValidator.DefaultValidatorConfig.RejectRuntimeDefaultSeccompAdjustment = boolptr(
			ctx.Bool("nri-validator-reject-runtime-default-seccomp-adjustment"),
		)
	}

	if ctx.IsSet("nri-validator-reject-unconfined-seccomp-adjustment") {
		config.NRI.DefaultValidator.DefaultValidatorConfig.RejectUnconfinedSeccompAdjustment = boolptr(
			ctx.Bool("nri-validator-reject-unconfined-seccomp-adjustment"),
		)
	}

	if ctx.IsSet("nri-validator-reject-custom-seccomp-adjustment") {
		config.NRI.DefaultValidator.DefaultValidatorConfig.RejectCustomSeccompAdjustment = boolptr(
			ctx.Bool("nri-validator-reject-custom-seccomp-adjustment"),
		)
	}

	if ctx.IsSet("nri-validator-reject-namespace-adjustment") {
		config.NRI.DefaultValidator.DefaultValidatorConfig.RejectNamespaceAdjustment = boolptr(
			ctx.Bool("nri-validator-reject-namespace-adjustment"),
		)
	}

	if ctx.IsSet("nri-validator-required-plugins") {
		config.NRI.DefaultValidator.DefaultValidatorConfig.RequiredPlugins = StringSliceTrySplit(ctx, "nri-validator-required-plugins")
	}

	if ctx.IsSet("nri-validator-tolerate-missing-plugins-annotation") {
		config.NRI.DefaultValidator.DefaultValidatorConfig.TolerateMissingAnnotation = ctx.String("nri-validator-tolerate-missing-plugins-annotation")
	}

...
func mergeConfig(config *libconfig.Config, ctx *cli.Context) error {
	...
	if ctx.IsSet("nri-socket-path") {
		config.NRI.SocketPath = ctx.String("nri-socket-path")
	}
...
func getCrioFlags(defConf *libconfig.Config) []cli.Flag {
    ...
    inserts by default (e.g. '/dev/shm') are not considered.`,
		...
		&cli.StringFlag{
			...
			Usage: fmt.Sprintf("Socket to create for externally started NRI plugins to connect to. (default: %q)", defConf.NRI.SocketPath),
		},
		&cli.StringFlag{
			Name:  "nri-plugin-dir",

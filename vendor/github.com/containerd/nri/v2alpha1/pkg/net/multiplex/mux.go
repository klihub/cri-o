...
import (
	...
	nrinet "github.com/containerd/nri/v2alpha1/pkg/net"
...
// if the connections incoming queue is empty. If a connections queue
// becomes full, new data demultiplexed for the connection is silently
// dropped until some queued data is consumed by reading.
//
...
// same ConnID is or will be opened at the other end of the Mux.
type Mux interface {
...
func (m *mux) Open(id ConnID) (net.Conn, error) {
	if id == reservedConnID {
		return nil, errors.Errorf("ConnID %d is reserved", id)
	}
...
func (m *mux) write(id ConnID, buf []byte) (int, error) {
	...
	if err != nil {
		err = errors.Wrap(err, "failed to write header to trunk")
		if n != 0 {
			m.setError(err)
			m.Close()
		}
	...
	if err != nil {
		err = errors.Wrap(err, "failed to write payload to trunk")
		if n != 0 {
			m.setError(err)
			m.Close()
		}
...
func (m *mux) reader() {
	...
	for {
		...
		if err != nil {
			m.setError(errors.Wrap(err, "failed to read header from trunk"))
			m.Close()
			return
		}
		...
		if err != nil {
			m.setError(errors.Wrap(err, "failed to read payload from trunk"))
			m.Close()
			return
		}

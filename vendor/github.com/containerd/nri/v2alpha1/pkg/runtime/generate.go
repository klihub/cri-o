...
type Generator struct {
	*generate.Generator
	...
	ensurePropagation func(string, ...string) error
}
...
func SpecGenerator(gg *generate.Generator, opts ...GeneratorOption) *Generator {
	...
	g.addLabel = g.AddAnnotation
	g.removeLabel = g.RemoveAnnotation
...
// WithLabelSetters can be used to alter the default label manipulation functions (annotations).
func WithLabelSetters(add func(string, string), remove func(key string)) GeneratorOption {
	return func(g *Generator) {
		g.addLabel = add
		g.removeLabel = remove
	}
}
...
// WithEnsurePropagation specifies the function to ensure propagation on/of a mount.
func WithEnsurePropagation(fn func(string, ...string) error) GeneratorOption {
	return func(g *Generator) {
		g.ensurePropagation = fn
	}
}

// Adjust adjusts all aspects of the OCI Spec that NRI knows/cares about.
func (g *Generator) Adjust(adjust *ContainerAdjustment) error {
	if adjust == nil {
		return nil
	}

	if err := g.AdjustLabels(adjust.GetLabels()); err != nil {
		return errors.Wrap(err, "failed to adjust labels in OCI Spec")
	}
	if err := g.AdjustAnnotations(adjust.GetAnnotations()); err != nil {
		return errors.Wrap(err, "failed to adjust annotations in OCI Spec")
	}
...
func (g *Generator) AdjustEnv(env []*KeyValue) {
	mod := map[string]*KeyValue{}

	for _, e := range env {
		key, _ := IsMarkedForRemoval(e.Key)
		mod[key] = e
	}
...
// AdjustLabels adjusts the labels in the OCI Spec.
func (g *Generator) AdjustLabels(labels map[string]string) error {
	var err error

	if labels, err = g.filterLabels(labels); err != nil {
		return err
	}
...
func (g *Generator) AdjustHooks(hooks *Hooks) {
	if hooks == nil {
		return
	}
...
func (g *Generator) AdjustResources(r *LinuxResources) error {
	if r == nil {
		return nil
	}
	...
	if g.checkResources != nil {
		if err := g.checkResources(g.Config.Linux.Resources); err != nil {
			return errors.Wrap(err, "failed to adjust resources in OCI Spec")
		}
	}
...
func (g *Generator) AdjustBlockIOClass(blockIOClass *string) error {
	...
	if err != nil {
		return errors.Wrap(err, "failed to adjust BlockIO class in OCI Spec")
	}
...
func (g *Generator) AdjustRdtClass(rdtClass *string) error {
	...
	if err != nil {
		return errors.Wrap(err, "failed to adjust RDT class in OCI Spec")
	}
...
func (g *Generator) AdjustDevices(devices []*LinuxDevice) {
...
func (g *Generator) AdjustMounts(mounts []*Mount) error {
	var (
		propagation string
		relabel     bool
	)
	...
	for _, m := range mounts {
		...
		mnt := m.ToOCI(&propagation, &relabel)
		switch propagation {
		case "rprivate":
		case "rshared":
			if err := g.ensurePropagation(mnt.Source, "rshared"); err != nil {
				return errors.Wrap(err, "failed to adjust mounts in OCI Spec")
			}
			if err := g.SetLinuxRootPropagation("rshared"); err != nil {
				return errors.Wrap(err, "failed to adjust rootfs propagation in OCI Spec")
			}
		case "rslave":
			if err := g.ensurePropagation(mnt.Source, "rshared", "rslave"); err != nil {
				return errors.Wrap(err, "failed to adjust mounts in OCI Spec")
			}
		...
		if relabel {
			if mntLabel := g.Config.Linux.MountLabel; mntLabel != "" {
				if err := label.Relabel(mnt.Source, mntLabel, false); err != nil {
					return errors.Wrap(err, "failed to adjust mounts in OCI Spec")
				}
			}
		}

...
// AddLabel records the addition of the label key=value.
func (a *ContainerAdjustment) AddLabel(key, value string) {
	a.initLabels()
	a.Labels[key] = value
}

// RemoveLabel records the removal of the label for the given key.
// Normally it is an error for a plugin to try and alter a label
// touched by another plugin. However, this is not an error if the
// plugin removes that label prior to touching it.
func (a *ContainerAdjustment) RemoveLabel(key string) {
	a.initLabels()
	a.Labels[MarkForRemoval(key)] = ""
}
...
// AddHook records the addition of the given hooks to a container.
func (a *ContainerAdjustment) AddHook(h *Hooks) {
	a.initHooks()
	if h.Prestart != nil {
...
func (a *ContainerAdjustment) DisableOOMKiller() {
	a.initLinuxResourcesMemory()
...
func (a *ContainerAdjustment) SetCPURTRuntime(value int64) {
	a.initLinuxResourcesCPU()
...
func (a *ContainerAdjustment) SetBlockIOClass(value string) {
	a.initLinuxResources()
	a.Linux.Resources.BlockioClass = String(value)
}

// SetRDTClass records setting the RDT class for a container.
func (a *ContainerAdjustment) SetRDTClass(value string) {
	a.initLinuxResources()
	a.Linux.Resources.RdtClass = String(value)
}

// SetCgroupsPath records setting the cgroups path for a container.
func (a *ContainerAdjustment) SetCgroupsPath(value string) {
	a.initLinux()
...
func (a *ContainerAdjustment) initLabels() {
	if a.Labels == nil {

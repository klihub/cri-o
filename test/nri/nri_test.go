...
func TestMountInjection(stdT *testing.T) {
	...
	out := "> /out/" + testFile
	cmd := WithShellScript("set -e; echo " + msg + out + "; sleep 3600")
...
func TestDeviceInjection(stdT *testing.T) {
	skipNriTestIfNecessary(stdT,
		map[string]bool{
			"user namespace enabled": *userNs == "1",
			"no runtime connection":  crio == nil,
		},
	)
...
func skipNriTestIfNecessary(t *testing.T, skipChecks ...map[string]bool) {
